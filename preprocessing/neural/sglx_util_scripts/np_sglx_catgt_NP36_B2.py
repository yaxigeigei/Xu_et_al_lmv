...
if sys.platform.startswith('linux'):
    ...
    src_root = Path('/datastore_spirit/human/Neuropixels')
    out_root = Path('/userdata/dxu/project_np/preproc/sglx')
    
    # Specify the original ap.bin path
    subj_id = 'NP36'
    block_name = 'B2'
    ap_bin_path = src_root / '{0}/{0}_{1}_g0/{0}_{1}_g0_imec0/{0}_{1}_g0_t0.imec0.ap.bin'.format(subj_id, block_name)
    ...
    # Source SpikeGLX data directory
    src_root = Path('C:/Users/many/Lab/preproc/sglx')
    out_root = src_root
...
if ap_bin_path.is_file():
    ...
    src_dir = ap_bin_path.parents[2]
    out_dir = out_root / run_base_name
...
# Assemble command
params = list()
params.append('-dir=' + str(src_dir))  # subject directory of source data, e.g. '/datastore_spirit/human/Neuropixels/NP11'
...
params.append('-dest=' + str(out_dir))      # recording directory for output data, e.g. '/userdata/dxu/project_np/preproc/sglx/NP11_B2'
params = ' '.join(params)
