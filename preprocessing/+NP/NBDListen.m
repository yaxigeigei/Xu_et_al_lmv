classdef NBDListen < NP.TaskBaseClass
    ...
    methods(Static)
        function [tv, tt] = AddTaskValueTable(se)
            ...
            assert(all(ismember(requiredCols, tt.Properties.VariableNames)), ...
                "NBDListen task requires the 'stim' column in the taskTime table.");
            ...
            if ~numTrials
                fprintf("This se does not have any nbd_listen trial\n");
                return
            end
            
            % Make unique numeric stim IDs
            m = tv.taskName == "nbd_listen";
        ...
        function PlotOverview(se, varargin)
            ...
            figName = strjoin([recId, "nbd_listen", "page"+p], "_"); %filename is often too long with all the unitIds included
            if ~exist(figDir, 'dir')
                mkdir(figDir);
            end
