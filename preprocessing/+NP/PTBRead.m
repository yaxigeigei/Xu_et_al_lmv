classdef PTBRead < NP.TaskBaseClass
    ...
    methods(Static)
        function [tv, tt] = AddTaskValueTable(se)
            ...
            NP.TaskBaseClass.MigrateStimId(se, '^.{5}_si.{3,}_read');
            ...
            assert(all(ismember(requiredCols, tt.Properties.VariableNames)), ...
                "PTBRead task requires the 'stim' column in the taskTime table.");
            ...
            if ~numTrials
                fprintf("This se does not have any PTBRead trial\n");
                return
            end
            
            % Make unique numeric stim IDs
            m = tv.taskName == "ptb_read";
        ...
        function [tt, tv] = FindKeyTimes(tt, tv)
            ...
            for i = 1 : height(tt)
                ...
                if silent_intervals
                    % Note that the even-index intervals in the TextGrid are empty strings corresponding to the transition periods between words.
        ...
        function [uId, uIdUni] = GetSelectedClusId(recId)
            ...
            % 
            switch recId
                case 'NP98_B1', uId = [980100184 980100165 980100789 980100620 980100101 980100022];
        ...
        function PlotOverview(se, varargin)
            ...
            panels2args = repmat({{{'firstWordOn', 'lastWordOff'}, @(x) brewermap(x, 'Dark2')}}, size(panels1));
            
            panels3 = repmat({'block_boundary'}, size(panels1));
            % panels3args = repmat({{{'arithmetic_secondOperand'},'Label', false}}, size(panels1));
            panels3args = repmat({{{'stimText'},'Label', false}}, size(panels1));
            ...
            figName = strjoin([recId, "timit_sorted", "page"+p], "_"); %filename is often too long with all the unitIds included
            if ~exist(figDir, 'dir')
                mkdir(figDir);
            end
        ...
        function seSort = SortTrials(se)
            ...
            % TODO: adapt for PTBRead. Will require editing TV.
            
            tv = se.GetTable('taskValue');
            
            % Make a categorical version of the sentence text
            tv.stimText_categorical = categorical(tv.stimText);
            
            % Sort se
            % [tv, I] = sortrows(tv, {'arithmetic_secondOperand'});
            [tv, I] = sortrows(tv, {'stimText_categorical'});
            seSort = se.Duplicate;
            seSort.SortEpochs(I);
        ...
        function seTb = SplitBySentence(se)
            ...
            % Remove trials that are not ptb_read
            tv = seLite.GetTable('taskValue');
            seLite.RemoveEpochs(lower(tv.taskName) ~= "ptb_read");
